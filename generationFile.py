...
import re
import sys
import syntax_tree_gen as st


class VsopParser3():


    def __init__(self, lexer, file_name, string_text, dictionaries, module, builder):
    ...
    def p_init(self, p):
        ...
        p[0] = st.Tree_node_gen(self.module, self.builder, "program", self.classes, [], pos)
    ...
    def p_class(self, p):
        ...
        pos = self.find_position(p, 1)
        if len(p) == 4:
            p[0] = st.Tree_node_gen(self.module, self.builder, "class", [st.Tree_node_gen(self.module, self.builder, "fields", self.fields, [], pos, self.current_class), st.Tree_node_gen(self.module, self.builder, "methods", self.methods, [], pos, self.current_class)], [p[2]], pos, self.current_class)
        else: 
            p[0] = st.Tree_node_gen(self.module, self.builder, "class", [st.Tree_node_gen(self.module, self.builder, "fields", self.fields, [], pos, self.current_class), st.Tree_node_gen(self.module, self.builder, "methods", self.methods, [], pos, self.current_class)], [p[2], p[4]], pos, self.current_class)
        self.fields = []
        self.methods = []
    ...
    def p_field(self, p):
        ...
        pos = p[1].position
        if len(p) == 3:
            p[0] = st.Tree_node_gen(self.module, self.builder, "field", [p[1]], [], pos, self.current_class, p[1].type)
        else:
            p[0] = st.Tree_node_gen(self.module, self.builder, "field", [p[1], p[3]], [], pos, self.current_class, p[1].type)
    ...
    def p_field_identifier(self, p):
        ...
        p[0] = st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], pos, self.current_class, p[3])

    def p_method(self, p):
        'method : method_identifier LPAR formals RPAR COLON type block'
        pos = self.find_position(p, 2)
        p[0] = st.Tree_node_gen(self.module, self.builder, "method", [st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], pos, self.current_class, p[6]), st.Tree_node_gen(self.module, self.builder, "formals", self.formals[-1], [], self.find_position(p, 4), self.current_class), p[7]], [], pos, self.current_class, p[6])
    ...
    def p_formal(self, p):
        ...
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "formal", [], [p[1]], pos, self.current_class, p[3])
    ...
    def p_block(self, p): 
        ...
        p[0] = st.Tree_node_gen(self.module, self.builder, "block", self.blocks[-1], [], pos, self.current_class, block_type)
        del self.blocks[-1]
    ...
    def p_if(self, p):
        ...
        pos = self.find_position(p, 1)
        if len(p) == 5:
            p[0] = st.Tree_node_gen(self.module, self.builder, "if", [p[2], p[4]], [], pos, self.current_class, "unit")
        else: 
            if self.unordered_compare_types(p[4].type, p[6].type):
                p[0] = st.Tree_node_gen(self.module, self.builder, "if", [p[2], p[4], p[6]], [], pos, self.current_class, p[6].type)
            else:
                p[0] = st.Tree_node_gen(self.module, self.builder, "if", [p[2], p[4], p[6]], [], pos, self.current_class, "unit")
    ...
    def p_while(self, p):
        ...
        if(p[2].type != "bool"):
            ...
            sys.exit(1)
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "while", [p[2], p[4]], [], pos, self.current_class, "unit")
    ...
    def p_let(self, p):
        ...
        pos = self.find_position(p, 1)
        if len(p) == 5:
            p[0] = st.Tree_node_gen(self.module, self.builder, "let", [p[2], p[4]], [], pos, self.current_class, p[4].type)
        else:
            if not self.compare_types(p[2].type, p[4].type):
                ...
                sys.exit(1)
            p[0] = st.Tree_node_gen(self.module, self.builder, "let", [p[2], p[4], p[6]], [], pos, self.current_class, p[6].type)
        self.variables_list.pop()

    def p_let_type(self, p):
        "let_type : OBJECT_IDENTIFIER COLON type"
        p[0] = st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], self.find_position(p, 1), self.current_class, p[3])
        self.add_variable(p[1], p[3])
    ...
    def p_assign(self, p):
        ...
        if t == "error":
            ...
            sys.stderr.write("{0}:{1}:{2}: semantic error: use of unbound variable {3}\n".format(self.file_name, pos[0], pos[1], p[1]))
            sys.exit(1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "assign", [st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], pos, self.current_class), p[3]], [], pos, self.current_class, p[3].type)
        self.add_variable(p[1], p[3])
    ...
    def p_unary_not(self, p):
        ...
        if(p[2].type != "bool"):
            ...
            sys.exit(1)
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "unop", [p[2]], [p[1]], pos, self.current_class, "bool")
    ...
    def p_unary_minus(self, p):
        ...
        if(p[2].type != "int32"):
            ...
            sys.exit(1)
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen("unop", [p[2]], [p[1]], pos, self.current_class, "int32")
    ...
    def p_binary_equal(self, p):
        ...
        if not self.compare_types(p[1].type, p[3].type):
            ...
            sys.exit(1)
        pos = self.find_position(p, 2)
        p[0] = st.Tree_node_gen(self.module, self.builder, "binop", [p[1], p[3]], [p[2]], pos, self.current_class, "bool")
    ...
    def p_binary_int_operators(self, p):
        ...
        if(p[3].type != "int32"):
            ...
            sys.exit(1)
        pos = self.find_position(p, 2)
        p[0] = st.Tree_node_gen(self.module, self.builder, "binop", [p[1], p[3]], [p[2]], pos, self.current_class, "int32")
    ...
    def p_object_call(self, p):
        ...
        if len(p) == 5:
            ...
            if index >= 0:
                ...
                sys.exit(1)
            p[0] = st.Tree_node_gen(self.module, self.builder, "call", [st.Tree_node_gen(self.module, self.builder, "self", [], ["self"], pos, self.current_class, self.current_class), st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], pos, self.current_class), st.Tree_node_gen(self.module, self.builder, "args", self.args[-1], [], self.find_position(p, 4), self.current_class)], [], pos, self.current_class, method_type)
        else: 
            ...
            if index >= 0:
                ...
                sys.exit(1)
            p[0] = st.Tree_node_gen(self.module, self.builder, "call", [p[1], st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[3]], self.find_position(p, 4), self.current_class), st.Tree_node_gen(self.module, self.builder, "args", self.args[-1], [], self.find_position(p, 6), self.current_class)], [], pos, self.current_class, method_type) 
        del self.args[-1]
    ...
    def p_new_type(self, p):
        ...
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "new type", [], [p[2]], pos, self.current_class, p[2])
    ...
    def p_expression_object(self, p):
        ...
        if t == "error":
            ...
            sys.stderr.write("{0}:{1}:{2}: semantic error: use of unbound variable {3}\n".format(self.file_name, pos[0], pos[1], p[1]))
            sys.exit(1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "object identifier", [], [p[1]], pos, self.current_class, t)

    def p_expression_self(self, p):
        'expression : SELF'
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "self", [], [p[1]], pos, self.current_class, self.current_class)
    ...
    def p_par_alone(self,p):
        ...
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "par alone", [], ["()"], pos, self.current_class, "unit")
    ...
    def p_string_literal(self, p):
        ...
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "string literal", [], [p[1]], pos, self.current_class, "string")
    
    def p_integer_literal(self, p):
        '''literal : INTEGER_LITERAL'''
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "integer literal", [], [p[1]], pos, self.current_class, "int32")

    def p_boolean_literal(self, p):
        '''literal : TRUE 
                    | FALSE'''
        pos = self.find_position(p, 1)
        p[0] = st.Tree_node_gen(self.module, self.builder, "boolean literal", [], [p[1]], pos, self.current_class, "bool")
